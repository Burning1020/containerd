...
type CreateOptions struct {
	...
	Options typeurl.Any
	// Metadata is used to pass key-value configs, which is not generated by `Options`,
	// to shim by rpc header protocol.
	// For instance, sandbox network namespace path.
...
// WithMetadata used to assign metadata configs of a sandbox.
func WithMetadata(key, value string) CreateOpt {
	return func(co *CreateOptions) error {
		if co.Metadata == nil {
			co.Metadata = make(map[string]string)
		}
		co.Metadata[key] = value
		return nil
	}
}
